    ...
    public unsafe class RSARNode : NW4RNode
    {
        ...
        public override void OnPopulate()
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                Helpers.DbgPrint("Root Index = " + hdr->_rootId);
                ...
                for (int x = 0; x < hdr->_numEntries; x++)
                {
                    ...
                    _symbCache[i][x] = *e;
                    Helpers.DbgPrint(String.Format("[{5}] {0}, {1}, {2} - {4}", e->_bit != -1 ? e->_bit.ToString().PadLeft(3) : "   ", e->_leftId != -1 ? e->_leftId.ToString().PadLeft(3) : "   ", e->_rightId != -1 ? e->_rightId.ToString().PadLeft(3) : "   ", e->_index != -1 ? e->_index.ToString().PadLeft(3) : "   ", new string(offset + stringOffsets[e->_stringId]), x.ToString().PadLeft(3)));
